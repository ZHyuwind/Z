    ...
    while img_start < det_range[1]:
        ...
        for batch_id in range(0, len(cropped_data), cfg.test_batch_size):  # (0,2,32)
            ...
            for i in range(start_id, end_id):
                img, input_pose_coord, input_pose_valid, input_pose_score, crop_info = generate_batch(cropped_data[i],
                                                                                                      stage='test')
                imgs.append(img)
            ...
            if cfg.flip_test:
                ...
                for (q, w) in cfg.kps_symmetry:
                    flip_input_pose_coords_w, flip_input_pose_coords_q = flip_input_pose_coords[:, w,
                                                                         :].copy(), flip_input_pose_coords[:, q,
                                                                                    :].copy()
                    flip_input_pose_coords[:, q, :], flip_input_pose_coords[:, w,
                                                     :] = flip_input_pose_coords_w, flip_input_pose_coords_q
                    flip_input_pose_valids_w, flip_input_pose_valids_q = flip_input_pose_valids[:,
                                                                         w].copy(), flip_input_pose_valids[:, q].copy()
